...
function App() {
    ...
    const [isConnected, setIsConnected] = useState(true); // Power state DEFAULT ON - bouton vert au démarrage
    const [isMuted, setIsMuted] = useState(true); // Micro INACTIF par défaut (mode rouge/fermé)
    const [isVideoOn, setIsVideoOn] = useState(false); // Video OFF par défaut - l'auto-start la démarrera
    ...
    // Printing workflow status (for top toolbar display) - REMOVED
    ...
    const isVideoOnRef = useRef(false); // Démarrer avec vidéo OFF - l'auto-start la démarrera
    ...
    useEffect(() => {
        ...
        if (isAuthenticated && socketConnected && micDevices.length > 0 && !hasAutoConnectedRef.current) {
            hasAutoConnectedRef.current = true;

            // Auto-connect to model with small delay for socket stability
            const timer = setTimeout(() => {
                ...
                // isConnected est déjà à true au démarrage (bouton power vert)
                socket.emit('start_audio', {
                    device_index: index >= 0 ? index : null,
                    device_name: deviceName,
                    muted: true // Démarrer avec micro INACTIF (l'utilisateur doit cliquer)
                });
                console.log("[AUTO-START] Modèle connecté, micro et caméra INACTIFS (attente clic utilisateur)");
            }, 500);
    ...
    useEffect(() => {
        ...
        socket.on('status', (data) => {
            ...
            if (data.msg === 'D.E.L.T.A Demarre') {
                setStatus('Modele Connecte');
            } else if (data.msg === 'D.E.L.T.A Arrete') {
                setStatus('Connected');
            }
        });
    ...
    const startVideo = async () => {
        try {
            ...
            // Initialize the transmission canvas with higher resolution for better vision
            if (!transmissionCanvasRef.current) {
                transmissionCanvasRef.current = document.createElement('canvas');
                transmissionCanvasRef.current.width = 1280;  // Augmenté pour meilleure reconnaissance
                transmissionCanvasRef.current.height = 720;  // Augmenté pour meilleure reconnaissance
    ...
    const predictWebcam = () => {
        ...
        if (!videoRef.current || !canvasRef.current || !isVideoOnRef.current) {
            console.log("[FRONTEND] predictWebcam arrêté - videoRef:", !!videoRef.current, "canvasRef:", !!canvasRef.current, "isVideoOn:", isVideoOnRef.current);
            return;
        }
        ...
        // TOUJOURS envoyer les frames si la vidéo tourne (pas de condition isConnected)
        if (frameCountRef.current % 30 === 0) {
            console.log("[FRONTEND] Envoi frame - Frame #", frameCountRef.current);
            ...
            if (transCanvas) {
                ...
                // Convert resized image to blob with higher quality for better vision
                transCanvas.toBlob((blob) => {
                    if (blob && socket && socket.connected) {
                        console.log("[FRONTEND] Frame envoyée via socket, taille:", blob.size);
                        socket.emit('video_frame', { image: blob });
                    } else {
                        console.log("[FRONTEND] Impossible d'envoyer - blob:", !!blob, "socket:", !!socket, "connected:", socket?.connected);
                    }
                }, 'image/jpeg', 0.85); // Higher quality for better object recognition
            } else {
                console.log("[FRONTEND] transCanvas est null!");
            }
        } else if (frameCountRef.current % 30 === 0) {
            console.log("[FRONTEND] Frame non envoyée - isConnected:", isConnected);
        }
    ...
    // Auto-start caméra pour la vision (FORCÉ au démarrage)
    useEffect(() => {
        console.log("[FRONTEND] useEffect auto-start caméra - isAuthenticated:", isAuthenticated, "webcamDevices:", webcamDevices.length);
        if (isAuthenticated && webcamDevices.length > 0) {
            console.log("[FRONTEND] Démarrage FORCÉ de la caméra...");
            const timer = setTimeout(() => {
                console.log("[FRONTEND] Appel de startVideo()");
                startVideo();
            }, 2000); // Attendre 2 secondes après l'authentification
            return () => clearTimeout(timer);
        }
    }, [isAuthenticated, webcamDevices]);
    ...
    const togglePower = () => {
        if (isConnected) {
            ...
            socket.emit('start_audio', { 
                device_index: index >= 0 ? index : null,
                muted: true // Démarrer avec le micro en pause
            });
            setIsConnected(true);
            setIsMuted(true); // Démarrer en mode mute - l'utilisateur doit activer le micro
        }
    };
    ...
    const toggleMute = () => {
        ...
        if (isMuted) {
            ...
            setIsMuted(false);
            addMessage('System', '🎤 Microphone activé - Mode vocal');
        } else {
            socket.emit('pause_audio');
            setIsMuted(true);
            addMessage('System', '✍️ Microphone désactivé - Mode écrit disponible');
        }
    };
    ...
    const handleImprove = () => {
        const improvementPrompt = "Je souhaite améliorer le projet DELTA. Peux-tu m'aider à identifier les améliorations possibles et les implémenter ?";
        socket.emit('user_input', { text: improvementPrompt });
        addMessage('User', improvementPrompt);
    };
