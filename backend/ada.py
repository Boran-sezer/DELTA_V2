...
from tts_edge import get_edge_tts
from ada_config import AUDIO_MODE, MODEL, DEFAULT_VIDEO_MODE
...
CHUNK_SIZE = 2048  # Augmenté pour audio plus fluide (était 1024)

# MODEL et DEFAULT_MODE sont maintenant importés de ada_config

load_dotenv()

# Gestionnaire Multi-Gemini pour quotas illimités
try:
    from gestionnaire_gemini import GestionnaireGemini
    gestionnaire_gemini = GestionnaireGemini()
    client = gestionnaire_gemini.obtenir_client()
    print("[DELTA] Systeme Multi-Gemini active")
except Exception as e:
    print(f"[DELTA] Fallback vers Gemini simple: {e}")
...
execute_command = {
    ...
    "description": "Exécute une commande système Windows (cmd). Exemples: 'dir', 'ipconfig', 'ping google.com', 'systeminfo'",
    "parameters": {
        "type": "OBJECT",
        "properties": {
...
# --- CONFIG: Configuration selon le mode audio ---
if AUDIO_MODE == "edge_tts" or AUDIO_MODE == "tts_local":
    # Mode TTS externe: Gemini envoie seulement du texte
    config = types.LiveConnectConfig(
        response_modalities=["TEXT"],  # Texte seulement
        output_audio_transcription={},  # Transcription pour TTS externe
        input_audio_transcription={},   # Reconnaissance vocale
        system_instruction=SYSTEM_INSTRUCTION,
        tools=tools
    )
else:
    # Mode streaming audio natif Gemini
    config = types.LiveConnectConfig(
        response_modalities=["AUDIO"],  # Audio natif
...
print(f"[CONFIG] Mode Audio: {AUDIO_MODE}")
...
class AudioLoop:
    def __init__(self, video_mode=DEFAULT_VIDEO_MODE, on_audio_data=None, on_video_frame=None, on_cad_data=None, on_web_data=None, on_transcription=None, on_tool_confirmation=None, on_cad_status=None, on_cad_thought=None, on_project_update=None, on_device_update=None, on_error=None, input_device_index=None, input_device_name=None, output_device_index=None):
    ...
    async def send_frame(self, frame_data):
        ...
        if isinstance(frame_data, bytes):
            ...
            print(f"[VISION] Frame convertie en base64: {len(b64_data)} chars")
        else:
            b64_data = frame_data
            print(f"[VISION] Frame déjà en base64: {len(b64_data) if isinstance(b64_data, str) else 'unknown'} chars")

        # Store as the designated "next frame to send" avec le bon format
        self._latest_image_payload = {"type": "image", "data": b64_data}
        print(f"[VISION] Frame stockée dans _latest_image_payload")
    ...
    async def send_realtime(self):
        while True:
            ...
            if msg_type == "audio":
                # Audio: utiliser send_realtime_input
                await self.session.send_realtime_input(
                    audio={"data": msg["data"], "mime_type": "audio/pcm"}
                )
            elif msg_type == "image":
                # Image: utiliser send_client_content avec turns
                print(f"[VISION] Envoi image à Gemini via send_client_content")
                await self.session.send_client_content(
                    ...
                    turn_complete=False  # Ne pas terminer le tour, juste ajouter l'image
                )

    async def send_frames_periodically(self):
        """Envoie les frames vidéo à Gemini toutes les 2 secondes"""
        print("[VISION] Démarrage de l'envoi périodique des frames...")
        while True:
            try:
                await asyncio.sleep(2.0)  # Attendre 2 secondes
                
                if self._latest_image_payload and self.out_queue:
                    print(f"[VISION] Envoi automatique de frame à Gemini")
                    await self.out_queue.put(self._latest_image_payload)
                else:
                    if not self._latest_image_payload:
                        print(f"[VISION] Pas de frame disponible pour envoi automatique")
                        
            except Exception as e:
                print(f"[VISION] Erreur dans send_frames_periodically: {e}")
    ...
    async def listen_audio(self):
        ...
        FRAME_SEND_COOLDOWN = 2.0  # Envoyer une frame max toutes les 2 secondes (au lieu de 5)
        ...
        while True:
            ...
            try:
                ...
                # 2. VAD Logic for Video (avec cooldown pour éviter surcharge)
                count = len(data) // 2
                if count > 0:
                ...
                if rms > VAD_THRESHOLD:
                    ...
                    if not self._is_speaking:
                        ...
                        print(f"[VAD] Parole détectée (RMS: {rms})")
    ...
    async def _execute_pc_command(self, command: str):
        """Exécute une commande PC via le PC Agent (API standard)"""
        print(f"[PC_AGENT] Exécution de: '{command}'")
        try:
            result = await self.pc_agent.execute_command(command)
            if result.get('success'):
                print(f"[PC_AGENT] ✅ Succès: {result.get('message', 'OK')}")
            else:
                print(f"[PC_AGENT] ❌ Erreur: {result.get('error', 'Unknown')}")
        except Exception as e:
            print(f"[PC_AGENT] ❌ Exception: {e}")
            import traceback
            traceback.print_exc()
            ...
            while True:
                ...
                async for response in turn:
                    ...
                    if data := response.data:
                        # Les données sont déjà en PCM brut (bytes)
                        ...
                        if response.server_content.input_transcription:
                            ...
                            if transcript:
                                ...
                                if transcript != self._last_input_transcription:
                                    ...
                                    if delta:
                                        # User is speaking - interrupt model playback only if significant speech
                                        # (plus de 3 caractères pour éviter de couper sur des petits bruits)
                                        if len(delta.strip()) > 3:
                            ...
                            if transcript:
                                ...
                                if transcript != self._last_output_transcription:
                                    ...
                                    if delta:
                                        # Send to frontend
                                        if self.on_transcription:
                                             self.on_transcription({"sender": "DELTA", "text": delta})
                                        
                                        # TTS Edge ou Local
                                        if AUDIO_MODE == "edge_tts":
                                            try:
                                                tts = get_edge_tts()
                                                tts.speak(delta)
                                            except Exception as e:
                                                print(f"[TTS] Erreur Edge TTS: {e}")
                                        elif AUDIO_MODE == "tts_local":
                                            try:
                                                tts = get_tts()
                                                tts.speak(delta)
                                            except Exception as e:
                                        ...
                                        # Buffer for Logging
                                        if self.chat_buffer["sender"] != "DELTA":
                                            if self.chat_buffer["sender"] and self.chat_buffer["text"].strip():
                                                self.project_manager.log_chat(self.chat_buffer["sender"], self.chat_buffer["text"])
                                            self.chat_buffer = {"sender": "DELTA", "text": delta}
                                        else:
                                            self.chat_buffer["text"] += delta
                    ...
                    if response.tool_call:
                        ...
                        for fc in response.tool_call.function_calls:
                            if fc.name in ["generate_cad", "run_web_agent", "write_file", "read_directory", "read_file", "create_project", "switch_project", "list_projects", "iterate_cad"]:
    ...
    async def play_audio(self):
        # Configuration comme ADA original: AUCUN pre-buffer, juste buffer PyAudio
        stream = await asyncio.to_thread(
            ...
            frames_per_buffer=CHUNK_SIZE,  # Taille originale
        )
        
        print(f"[AUDIO] Configuration ADA originale: streaming direct sans pre-buffer")
    ...
    async def get_frames(self):
        # Utiliser CAP_DSHOW sur Windows pour une meilleure compatibilité
        import platform
        ...
        while True:
            ...
            # Mettre à jour le payload pour que VAD puisse l'utiliser
            self._latest_image_payload = frame
            print("[CAMERA] Frame captured and stored")
        ...
        while not self.stop_event.is_set():
            try:
                ...
                async with (
                    ...
                    tg.create_task(self.send_frames_periodically())  # Envoi automatique des frames

                    # Vision: Utiliser les frames du frontend (plus fiable sur Windows)
                    # Les frames sont envoyées via socket.io et stockées dans _latest_image_payload
                    print("[VISION] Utilisation des frames du frontend (caméra web)")
                    # Note: Le frontend doit activer la caméra et envoyer les frames
